...
- 已确认当前 `upcoming_query` 仍仅覆盖单次任务；阶段 4 首个实现目标为补齐查询窗口内的重复实例展开与稳定 `occurrence_key`。

执行清单：

- `进行中` 定义阶段 4 首版支持范围与最小重复规则模型，明确当前只做查询窗口展开，不提前进入单次覆盖和模板截断。
- `未开始` 实现按天重复展开。
- `未开始` 实现按周重复展开。
- `未开始` 实现按月重复展开。
- `未开始` 实现按年重复展开。
- `未开始` 实现间隔值与循环截止日期支持。
- `未开始` 实现稳定 `occurrence_key` 生成规则。
- `未开始` 接入 `upcoming_query` 的实例展开投影。
- `未开始` 补充重复规则展开的 Rust 单元测试。
